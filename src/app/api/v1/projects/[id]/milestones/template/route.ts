...
import { initMilestoneTemplateSchema } from "@/lib/validations/project";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = initMilestoneTemplateSchema.parse(body);
    const meta = await getRequestMeta();

    const result = await ProjectMilestoneService.initializeTemplate(actor, {
      projectId: id,
